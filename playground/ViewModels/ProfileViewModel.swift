...
final class ProfileViewModel {
    ...
    func autoGenerateMacros() async {
        ...
        do {
            ...
            await MainActor.run {
                // Store as recommended goals
                UserSettings.shared.updateRecommendedGoals(
                ...
                // Update current goals using the tracking method
    ...
    private func syncNutritionGoalsToUserSettings() {
        ...
        // Use updateGoals to properly track previous values and metadata
        settings.updateGoals(
            calories: calorieGoal,
            protein: proteinGoal,
            carbs: carbsGoal,
            fat: fatGoal,
            isAutoGenerated: false // Manual changes are not auto-generated
        )
    }
    
    // MARK: - Goal Status Computed Properties
    
    /// Whether current goals are customized (different from recommended)
    var goalsAreCustomized: Bool {
        UserSettings.shared.goalsAreCustomized
    }
    
    /// Whether current goals were auto-generated
    var goalsAreAutoGenerated: Bool {
        UserSettings.shared.goalsAreAutoGenerated
    }
    
    /// Formatted text for when goals were last updated
    var goalsLastUpdatedText: String? {
        UserSettings.shared.goalsLastUpdatedText
    }
    
    /// Goal status text for display ("Custom" or "Auto")
    var goalStatusText: String {
        goalsAreAutoGenerated ? "Auto" : "Custom"
    }
    
    /// Differences from recommended values
    var recommendedDifferences: (calories: Int, protein: Double, carbs: Double, fat: Double) {
        UserSettings.shared.recommendedDifferences
    }
    
    /// Recommended goals from UserSettings
    var recommendedCalorieGoal: Int {
        UserSettings.shared.recommendedCalorieGoal
    }
    
    var recommendedProteinGoal: Double {
        UserSettings.shared.recommendedProteinGoal
    }
    
    var recommendedCarbsGoal: Double {
        UserSettings.shared.recommendedCarbsGoal
    }
    
    var recommendedFatGoal: Double {
        UserSettings.shared.recommendedFatGoal
    }
